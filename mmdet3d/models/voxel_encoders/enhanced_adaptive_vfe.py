# Enhanced Adaptive VFE for Multi-Resolution Processing
# This version provides richer adaptive information to the multi-resolution sparse encoder
...
class EnhancedAdaptiveVFE(nn.Module):
    """
    Enhanced Adaptive Voxel Feature Encoder that provides rich adaptive information
    for multi-resolution sparse processing.
    """
    def __init__(self,
                 in_channels,
                 feat_channels,
        ...
        self.provide_multi_res_info = provide_multi_res_info
        ...
        # Enhanced feature processing for multi-resolution
        ...
        # Additional networks for multi-resolution support
        if provide_multi_res_info:
            ...
            self.density_classifier = nn.Sequential(
                nn.Linear(1, 16),  # Just point count
                nn.ReLU(),
                nn.Linear(16, 8),
                nn.ReLU(),
                nn.Linear(8, 3),  # Low, medium, high density
                nn.Softmax(dim=-1)
            )
    ...
    def compute_multi_resolution_info(self, features, num_points):
        """Compute additional information for multi-resolution processing."""
        if not self.provide_multi_res_info:
            return {}
        ...
        complexity_input = torch.cat([voxel_means, normalized_counts], dim=1)
        complexity_scores = self.complexity_predictor(complexity_input).squeeze(1)
        
        # Classify density levels
        density_input = normalized_counts
        density_probs = self.density_classifier(density_input)
        
        # Compute feature variance as geometric complexity measure
        if features.shape[1] > 1:  # More than one point
            feature_variance = features.var(dim=1).mean(dim=1)  # Average variance across dimensions
        else:
            feature_variance = torch.zeros(features.shape[0], device=features.device)
        ...
        return {
            ...
            'point_density': normalized_counts.squeeze(1)
        }
    ...
    def forward(self, features, num_points, coors):
        ...
        # Compute multi-resolution information
        multi_res_info = self.compute_multi_resolution_info(features, num_points)
        adaptive_info.update(multi_res_info)
        ...
        if self.training and torch.rand(1).item() < 0.01:
            if adaptive_sizes.dim() > 1:
                ...
                print(f"Enhanced Adaptive voxel sizes - Mean: {mean_sizes.detach().cpu().numpy()}")
                print(f"Enhanced Adaptive voxel sizes - Std: {std_sizes.detach().cpu().numpy()}")
                if 'complexity_scores' in multi_res_info:
                    print(f"Complexity scores - Mean: {multi_res_info['complexity_scores'].mean():.3f}")
            else:
                print(f"Enhanced Adaptive voxel sizes: {adaptive_sizes.detach().cpu().numpy()}")
